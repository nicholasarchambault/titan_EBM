...
#         Weighting function from Siler et al., (in rev as of 6/8/2018)         #
#                          ------------------------                             #
...
hydrofun <- function(temp, x, hadley_extent=15){
  ...
  x15 <- sin((pi/180)*(hadley_extent))         # find the x value equal to the latitude of prescribed hadley cell extent
  ...
  myout <- as.data.frame(cbind(temp, h, Prec, Evap, E_m_P, flux, F_LH, F_DS, F_LH_eddy, F_LH_hadley, divF_LH))
